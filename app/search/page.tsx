...
import { Input_Range_Price_Slider } from "../../components/FiltersCard";
...
// const MOCK_VENUES = Array.from({ length: 6 }).map((_, i) => ({
//   id: `venue-${i}`,
//   name: "Detailed Venue Name",
...
export default function SearchPage() {
  ...
  function venueTypeTokens(raw: string) {
    // "Live Music Venue, Restaurant" -> ["live music venue", "restaurant"]
    return raw
      .split(",")
      .map((x) => x.trim())
      .filter(Boolean)
  ...
  const filtered_venues = useMemo(() => {
    ...
    return MOCK_VENUES.filter(v => {
      ...
      // Match if venue location is one of selected
  ...
  return (
    <main className="relative min-h-[calc(100vh-4rem)] overflow-hidden">
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-6 pb-16 pt-12">
        ...
        {query
            ? <>Your AI Powered Results for '{query}'</>
